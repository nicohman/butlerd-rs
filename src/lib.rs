...
impl Butler {
    pub fn new () -> Butler {
         let ch = Command::new("sh").arg("-c").arg("butler daemon --json --dbpath=".to_string()+&DB_PATH.replace("~", &get_home())+" --destiny-pid="+&::std::process::id().to_string()+" > /tmp/butlerdrs.log").spawn().expect("Couldn't start butler daemon");
    ...
    fn request(&self, method:Method, path:String) -> Result<String, String> {
        ...
        let mut res = self.client.request(method, &url).headers(headers).body("{}").send().unwrap();
    ...
    pub fn fetchall(&self) -> Vec<Cave> {
        let cvs = self.request(Method::Post, "/call/Fetch.Caves".to_string()).unwrap();
        let mut cavesR:FetchCaves = serde_json::from_str(&cvs).unwrap();
        let mut caves = caves.result["items"].as_array().unwrap().iter().map(|x| {
